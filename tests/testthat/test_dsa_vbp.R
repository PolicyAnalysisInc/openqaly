...
build_shared_price_dsa_vbp_model <- function() {
  define_model("markov") %>%
    set_settings(
      timeframe = 1,
    ...
    add_dsa_variable("utility", low = 1.5, high = 2.5, strategy = "treatment")
}
...
test_that("DSA+VBP applies a global price row override to all strategies", {
  model <- build_shared_price_dsa_vbp_model()
  ...
  segments <- openqaly:::build_dsa_vbp_segments(parsed, spec)
