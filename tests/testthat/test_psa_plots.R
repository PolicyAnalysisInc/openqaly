...
test_that("incremental_ceac_plot() show_frontier adds geom layer", {
  results <- get_cached_psa_results()

  p_without <- incremental_ceac_plot(
    results, "total_qalys", "total_cost",
    show_frontier = FALSE
  )
  p_with <- incremental_ceac_plot(
    results, "total_qalys", "total_cost",
    show_frontier = TRUE
  )
  ...
  # With frontier should have more layers

expect_gt(length(built_with$data), length(built_without$data))
})

# ============================================================================
# Tests for incremental_ceac_frontier_plot()
# ============================================================================

test_that("incremental_ceac_frontier_plot() returns ggplot object", {
  results <- get_cached_psa_results()
  p <- incremental_ceac_frontier_plot(results, "total_qalys", "total_cost")
  expect_s3_class(p, "ggplot")
})

test_that("incremental_ceac_frontier_plot() data has expected structure", {
  results <- get_cached_psa_results()

  p <- incremental_ceac_frontier_plot(
    results, "total_qalys", "total_cost",
    wtp_range = c(0, 100000), wtp_step = 25000
  )
  built <- ggplot_build(p)

  # Should have at least one data layer
  expect_true(length(built$data) >= 1)
  # First layer should have x (wtp) and y (optimal_strategy as factor level)
  expect_true("x" %in% names(built$data[[1]]))
  expect_true("y" %in% names(built$data[[1]]))
})

test_that("incremental_ceac_frontier_plot() show_probability changes aesthetics", {
  results <- get_cached_psa_results()

  p_without <- incremental_ceac_frontier_plot(
    results, "total_qalys", "total_cost",
    show_probability = FALSE
  )
  p_with <- incremental_ceac_frontier_plot(
    results, "total_qalys", "total_cost",
  ...
  # With probability should have alpha aesthetic
  expect_true("alpha" %in% names(built_with$data[[1]]))
})
