...
discontinuation_model <- define_model("custom_psm") |>
  ...
  add_transition("pf_on_tx", S_pfs * S_on_tx) |>
  # PF Off Tx: PFS survival * discontinued treatment
  add_transition("pf_off_tx", S_pfs * (1 - S_on_tx)) |>
  # Progressed: alive but not PF (OS - PFS area)
  add_transition("progressed", S_os - S_pfs) |>
...
```{r disc-costs, fig.width=9, fig.height=6}
costs_plot_bar(disc_results, outcome = "total_cost")
```

```{r disc-costs-table}
costs_table(disc_results, outcome = "total_cost")
```
...
response_model <- define_model("custom_psm") |>
  ...
  add_transition("response", p_response * S_dor * S_os_response) |>
  # Refractory: non-responders still alive
  add_transition("refractory", (1 - p_response) * S_os_refractory) |>
  # Relapse: responders who relapsed (simplified - in practice would need more complex modeling)
  add_transition("relapse", p_response * (1 - S_dor) * S_os_response) |>
  # Dead: complement
  add_transition("dead", C) |>
  # Values - costs
...
```{r response-costs, fig.width=9, fig.height=6}
costs_plot_bar(response_results, outcome = "total_cost")
```

```{r response-costs-table}
costs_table(response_results, outcome = "total_cost")
```
...
- **model start values**: Applied regardless of state (`state = NA`)
...
| Values | Residency + transitional | Residency + model start only |
