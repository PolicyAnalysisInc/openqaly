...
By default, tornado plot labels auto-generate as "Parameter Name (0.01 - 0.05)". Use `range_label` to override the range portion with custom text.

```{r, eval = FALSE}
model <- model %>%
  add_dsa_variable("p_mild", low = bc * 0.5, high = bc * 1.5,
                   range_label = "Base Case ± 50%")
```

This produces labels like "p_mild (Base Case ± 50%)" instead of "p_mild (0.015 - 0.045)".
...
#### Custom range label for settings

Settings also support `range_label` for custom tornado label text.
...
#### Showing only the top N parameters

Use `top_n` to limit the tornado plot to the most impactful parameters. This is useful for models with many DSA parameters.
...
The `dsa_costs_plot()` function creates tornado plots for cost summaries with currency formatting.
...
Use `top_n` to limit the table to the most impactful parameters.
...
#### Including input parameter values in labels

Use `show_parameter_values = TRUE` to append input range values to parameter labels, matching the tornado plot label format. When a parameter has a custom `range_label`, it is used instead of auto-generated values.
...
The `dsa_costs_table()` function presents DSA cost results with currency formatting.
