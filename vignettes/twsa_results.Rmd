...
```{r, include = FALSE}
# Configure TWSA with cost + utility for true 2D NMB/CE sensitivity
model_ce <- model |>
  add_twsa("Cost vs Utility") |>
  add_twsa_variable("Cost vs Utility", "c_treatment",
    type = "range", min = 5000, max = 15000, steps = 5,
    display_name = "Treatment Cost", strategy = "volantor") |>
  add_twsa_variable("Cost vs Utility", "u_mild",
    type = "range", min = 0.75, max = 0.95, steps = 5,
    display_name = "Utility (Mild)")
